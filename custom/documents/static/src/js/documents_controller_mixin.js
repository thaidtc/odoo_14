...
const DocumentsControllerMixin = Object.assign({}, fileUploadMixin, {
    events: {
        "click .o_documents_kanban_share_domain":
        ...
        "click .o_documents_toggle_workspace": "_onToggleWorkspace",
    ...
    custom_events: Object.assign({}, fileUploadMixin.custom_events, {
        ...
        document_viewer_attachment_changed:
            "_onDocumentViewerAttachmentChanged",
    ...
    init(parent, model, renderer, params) {
        ...
        session
            .user_has_group("documents.group_documents_user")
            .then((hasGroup) => {
                this.has_group = hasGroup;
            });
    },
    ...
    async reload() {
        ...
        const folderIds = [
            ...new Set(
                Object.values(this._fileUploads).map(
    ...
    renderButtons($node) {
        this.$buttons = $(
            qweb.render("DocumentsViews.buttons", { hasGroup: this.has_group })
        );
        if ($node) {
        ...
        return {
            ...
            targetCallback: (fileUpload) => {
                const $targetCard = this.$(
                    `.o_documents_attachment[data-id="${fileUpload.recordId}"]`
                );
     ...
     */
    _getRules(documents) {
        const ruleIdsByRecord = documents.map(
            (record) => record.available_rule_ids.res_ids
        );
        const commonRuleIds = _.intersection.apply(_, ruleIdsByRecord);
        const record = documents[0];
        const rules = commonRuleIds.map((ruleId) => {
            const rule = record.available_rule_ids.data.find(
                (record) => record.res_id === ruleId
            );
            return rule.data;
        });
    ...
    _onToggleWorkspace(){
        $(".o_documents_content .o_search_panel").toggle();
    },
    /**
    ...
    _makeDragIcon({ dataTransfer, lockedCount, draggedRecordIds }) {
        let dragIconContent;
        if (lockedCount > 0) {
            dragIconContent = _.str.sprintf(
                _t("%s Documents (%s locked)"),
        ...
        if (draggedRecordIds.length === 1) {
            ...
            const record = state.data.find(
                (record) => record.res_id === draggedRecordIds[0]
            );
            if (record) {
                dragIconContent = record.data.name
                    ? record.data.display_name
                    : _t("Unnamed");
            }
        }
        const $dragIcon = $(
            qweb.render("documents.dragIcon", {
                dragIconContent,
            })
        ).appendTo($("body"));
    ...
    async _renderChatter() {
        ...
        const target = $chatterContainer[0].querySelector(
            ":scope .o_documents_chatter_placeholder"
        );
    ...
    async _update(state, { controllerState: { selectedRecordIds } = {} } = {}) {
        ...
        this._selectedRecordIds = _.intersection(
    ...
    updateButtons() {
        ...
        this.$buttons
            .find(".o_documents_kanban_upload")
            .prop("disabled", !selectedFolderId);
        this.$buttons
            .find(".o_documents_kanban_url")
            .prop("disabled", !selectedFolderId);
        this.$buttons
            .find(".o_documents_kanban_request")
            .prop("disabled", !selectedFolderId);
        this.$buttons
            .find(".o_documents_kanban_share_domain")
            .prop("disabled", !selectedFolderId);
    },
    /**
    ...
    async _updateChatter() {
        ...
        if (this._selectedRecordIds.length === 1) {
            ...
            const chatterContainer = this.el.querySelector(
                ":scope .o_document_chatter_container"
            );
            const target = chatterContainer.querySelector(
                ":scope .o_documents_chatter_placeholder"
            );
            await this._chatterContainerComponent.mount(target);
        } else {
    ...
    _uploadFilesHandler(multiple) {
        return (ev) => {
            ...
            const $uploadInput = this.hiddenUploadInputFile
                ...
                : (this.hiddenUploadInputFile = $("<input>", {
                      type: "file",
                      name: "files[]",
                      class: "o_hidden",
                  }).appendTo(this.$el));
            ...
            $uploadInput.on("change", async (changeEv) => {
                await this._uploadFiles(changeEv.target.files, {
                    recordId,
                }).finally(cleanup);
            });
    ...
    _onClickDocumentsRequest(ev) {
        ...
        this.do_action("documents.action_request_form", {
            additional_context: {
                ...
                default_tag_ids: [
                    [6, 0, this.searchModel.get("selectedTagIds")],
                ],
            },
            fullscreen: config.device.isMobile,
    ...
    _onDragstartDocumentDraggable(ev) {
        ...
        switch (this.viewType) {
            case "kanban":
                isTargetSelected =
                    ev.currentTarget.classList.contains("o_record_selected");
                resId = $(ev.currentTarget).data("id");
                break;
            case "list":
                isTargetSelected = $(ev.currentTarget)
                    .find(".o_list_record_selector input")
                    .prop("checked");
                resId = $(ev.currentTarget)
                    .closest(".o_data_row")
                    .data("res-id");
                break;
        }
        ...
        const unlockedRecordIds = this.model
            .get(this.handle, { raw: true })
            .data.filter(
                (record) =>
                    !record.data.lock_uid ||
                    record.data.lock_uid === session.uid
            )
            .map((record) => record.res_id);
        const draggedRecordIds = _.intersection(
            this._selectedRecordIds,
            unlockedRecordIds
        );
        ...
        const lockedCount =
            this._selectedRecordIds.length - draggedRecordIds.length;
    ...
    async _onSaveMulti(ev) {
        ev.stopPropagation();
        try {
            await this.model.saveMulti(
                ev.data.dataPointIds,
                ev.data.changes,
                this.handle
            );
            await this.reload({});
    ...
    async _onSelectRecord(ev) {
        ...
        const { newSelection, anchor } = computeMultiSelection(
            recordIds,
            ev.data.resId,
            {
                ...
                isKeepSelection:
                    ...
                    ev.data.originalEvent.ctrlKey ||
                    ev.data.originalEvent.metaKey,
                isRangeSelection:
                    ev.data.originalEvent.shiftKey && this._anchorId,
